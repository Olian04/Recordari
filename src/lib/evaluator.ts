...
const unique: {
  ...
  [NodeType.Each]: (_eval, value, {children}) => value.reduce((res, val) => res && _eval(val, children[0]), true),
  [NodeType.Contains]: (_eval, value, {children}) => value.reduce((res, val) => res || _eval(val, children[0]), false),
  [NodeType.And]: (_eval, value, {children}) =>  children.reduce((res, child) => res && _eval(value, child), true),
  [NodeType.Or]: (_eval, value, {children}) => children.reduce((res, child) => res || _eval(value, child), false),
